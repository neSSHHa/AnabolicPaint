...
class ResizableCanvasApp:
    def __init__(self, root):
        ...
        # Create a canvas widget initially placed in the top-left corner
        ...
        self.canvas.pack(anchor="nw")  # Place the canvas in the top-left corner
        self.pasted_items = []  # Store all pasted items
        # Create initial dots for visual design
        self.dots = []
        self.pasted_images = [] 
        self.rectangle = None
        self.MouseInfo1 = (0, 0)
        ...
        # Serialized selection data
        self.serialized_selection = None

        # Flag for move/apply mode
        self.move_mode = False

        # Image item for pasted image
        self.pasted_image_item = None

        # Dashed border for the selection
        self.dashed_border = None

        # Blue border for the moving object
        self.blue_border = None

     
        # Bind mouse events for resizing and moving
        ...
        self.resize_threshold = 5  # Set your desired threshold value here
        self.min_canvas_size = 50  # Set the minimum canvas size
...
#aweawdasdsa
    ...
    def open_thickness_dialog(self):
        response = simpledialog.askinteger(
            ...
            minvalue=1,
            maxvalue=100,
            parent=self.root  # Specify the parent window
        )
...
#awdawdadsdwasdawda
    ...
    def create_dots(self):
        ...
        # Create new dots at the corners
    ...
    def start_resizing_action(self, event):
        ...
        self.last_x = mx
        # Check if the mouse is near any of the canvas edges or corners
    ...
    def resize_left(self, event):
        if not self.resizing:
            self.resizing = True
            self.last_x = event.x  # Update last_x here

        current_width = self.canvas.winfo_width()
        mx = event.x

        delta_x = mx - self.last_x  # Calculate the change in x position
        ...
        if new_width >= self.min_canvas_size:
            ...
            # Adjust the mouse position so that it doesn't go beyond the canvas boundary
    ...
    def start_selection(self, event):
        # Store the starting point of the selection
            ...
            # Clear the previous selection rectangles
            self.clear_selection()

    def update_selection(self, event):
        # Update the ending point of the selection as the mouse moves
    ...
    def clear_selection(self):
        # Delete the previous selection rectangle and dashed border
        if self.border:
            self.canvas.delete(self.border)

    def draw_selection(self):
        if self.active_select:
        # Clear the previous selection rectangle and dashed border
            self.clear_selection()

            # Adjust the coordinates for rendering the red border
            ...
            # Ensure that the selection rectangle does not go beyond the canvas boundaries
            x1 = max(x1, 0)
            x2 = min(x2, self.canvas_width)
            y1 = max(y1, 0)
            y2 = min(y2, self.canvas_height)

            # Draw the selection rectangle only if the mouse is inside the canvas boundaries
    ...
    def cut_selected(self):
        if self.border:
            ...
            self.canvas.itemconfigure(self.border, state=tk.HIDDEN)
            self.canvas.itemconfigure(self.dashed_border, state=tk.HIDDEN)
            self.canvas.update_idletasks()  # Update the canvas to hide the border
    ...
    def paste_selection(self):
        if self.serialized_selection:
            ...
            # Append the pasted item to the list
            self.pasted_items.append((pasted_image_item, pasted_image))

            # Bind mouse events for moving the last pasted item
            self.canvas.unbind("<B1-Motion>")
            self.canvas.unbind("<ButtonRelease-1>")
    ...
    def start_move(self, event, item):
        # Store the initial position for moving the image
